...
It does not create an OS scheduler job, daemon, retry path, takeover path,
...
caller after Discord target and local state paths pass preflight. Bootstrap
should use the host OS scheduler, not OpenClaw agent-turn cron: macOS user
LaunchAgent, Linux systemd user timer when available, and cron only when that is
the practical native binding. The recurring command must avoid positive
OK-message noise:
...
screen should show the scheduler kind, scheduler artifact path, command, target,
interval, stdout/stderr log paths, and alert state path before writing or
loading the job.
