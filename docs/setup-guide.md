...
Company Ops is actively monitoring on a schedule. Use the host OS scheduler:
macOS user LaunchAgent, Linux systemd user timer when available, and cron only
when it is the practical native binding. The recurring command must use
alert-only Discord delivery:
...
Before bootstrap writes or loads a scheduler registration, it must show the
operator the scheduler kind, artifact path, command, interval, target, log
paths, and state path, then require explicit confirmation.
